...
sdk = mag3nt.Mag3nt(
    ...
    security=Security(bearer_auth="sx_live_your_api_key_here"),
)
```
</CodeGroup>
...
Use your card to make a payment. Here's an example using x402:
...
```
```python Python
payment = sdk.x402.x402_pay(
    card_id=card.card.id,
    card_token=card.card.token,
    amount=0.50,
    merchant="weather-api.com",
    resource_url="https://weather-api.com/forecast",
)

print("Payment headers:", payment.headers)
# Attach these headers to retry your original 402 request
```
</CodeGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Payment protocols" icon="arrows-left-right" href="/concepts/protocols">
